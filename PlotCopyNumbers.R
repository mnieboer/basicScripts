...
for(row in 1:dim(EC70_CN)[1]){
  points(EC70_CN$plotx[row], EC70_CN$cn[row], pch=16, cex=0.75, col="blue")
}
...
abline(v=c(hg19.csize$offset, sum(as.numeric(hg19.csize$length))), col="grey") #vertical lines to separate chromosomes
legend("topright", c("SNP", "somatic SNV"), pch=c(16,1), col=c("black", "red"), cex = 0.75)
...
plot(NA, type= "p", xlim=c(hg19.csize[pstart,2],sum(as.numeric(hg19.csize$length)[1:pend])),
     ylim=c(0, 7), xaxt="n", yaxt="n",  cex=2, ylab="Copy number", xlab="",  cex.lab=1,main= "T3209_TE74" ) #
...
for(row in 1:dim(TE74_CN)[1]){
  points(TE74_CN$plotx[row], TE74_CN$cn[row], pch=16, cex=0.75, col="blue")
}
...
plot(NA, type= "p", xlim=c(hg19.csize[pstart,2],sum(as.numeric(hg19.csize$length)[1:pend])),
     ylim=c(0, 0.8), xaxt="n", yaxt="n",  cex=2, ylab="AF/LAF", xlab="",  cex.lab=1,main= "T3209_TE74" ) #
